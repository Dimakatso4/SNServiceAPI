...
namespace SNServiceAPI.Controllers.v1
{
    ...
    public class SupplierSummaryController : ControllerBase
    {
        ...
        public async Task<int> SaveSupplierDelivery(PerishableModel data)
        {
            ...
            dbparams.Add("@DocumentPath1", data.DocumentPath);
            dbparams.Add("@DocumentPath2", data.DocumentPath);
            dbparams.Add("@WrongQuality", data.WrongQuality);
            dbparams.Add("@WrongQuantity", data.WrongQuantity);
        ...
        public Task<SupplierSummaryModel> GetSupplierSummaryById(int Id)
        {
            ...
            var resultPerishable = Task.FromResult(_dapper.GetAll<PerishableModel>($"SELECT [PerishableProductId],fg.[FoodGroupId],fg.[FoodGroupDescription],tt.TypeID , tt.TypeDescription,[ProductId],[Quantity],[Unit],[SupplierSummaryId],[Grammage],[EmisNumber],[IsQualityWrong],[IsProductWrong],[IsQuantityWrong],[ItemDelieveryDate], [WrongProductName], [DocumentPath], [quantityRecieved] FROM[dbo].[tblPerishableProduct] inner join  tblFoodGroup as fg on fg.FoodGroupId = tblPerishableProduct.FoodGroupID  inner join  [tblType] as tt on tt.[TypeID] = tblPerishableProduct.TypeID WHERE [SupplierSummaryId] =  {Id}", null,
                    commandType: CommandType.Text));
